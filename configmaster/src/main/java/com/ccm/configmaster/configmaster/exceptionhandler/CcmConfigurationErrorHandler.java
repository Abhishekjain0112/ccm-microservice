...
public class CcmConfigurationErrorHandler {
	...
	public ErrorResponse handleExceptionValidation(Exception ex, HttpServletRequest request) {
		return new ErrorResponse(new Date(), HttpStatus.NOT_FOUND.value(), HttpStatus.NOT_FOUND.getReasonPhrase(), "Please check the Validation :"+ex.getMessage(), request.getRequestURI());
	}	
	...
	public ErrorResponse handleExceptionConstraintValidation(Exception ex, HttpServletRequest request) {
		return new ErrorResponse(new Date(), HttpStatus.CONFLICT.value(), HttpStatus.CONFLICT.getReasonPhrase(), "Duplicate Entry for Unique Key:"+ex.getMessage(), request.getRequestURI());
	}	
	...
	public ErrorResponse handleExceptionGlobal(Exception ex, HttpServletRequest request) {
		return new ErrorResponse(new Date(), HttpStatus.BAD_REQUEST.value(), HttpStatus.BAD_REQUEST.getReasonPhrase(), "Global Exception Handler: "+ex.getMessage(), request.getRequestURI());
	}	
